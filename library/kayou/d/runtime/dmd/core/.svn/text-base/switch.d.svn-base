...
int _d_switch_string(char[][] table, char[] ca)
	...
	for (j = 1; j < table.length; j++)
	{
		...
		ci = runtime.system.system.memcmp(table[j - 1].ptr, table[j].ptr, len1);
	...
	if (result == -1)
	{
		...
		{   cj = runtime.system.system.memcmp(table[i].ptr, ca.ptr, ca.length);
		    assert(cj != 0);
		}
	    }
	}
	    ...
	    ca.length <= table[high - 1].length)
	{
		...
		if (c == 0)
		{
		    ...
		    if (c == 0)
		    {
			c = runtime.system.system.memcmp(ca.ptr, pca.ptr, ca.length);
			if (c == 0)
			{   //printf("found %d\n", mid);
			    return mid;
			}
...
int _d_switch_ustring(wchar[][] table, wchar[] ca)
	...
	for (j = 1; j < table.length; j++)
	{
		...
		c = runtime.system.system.memcmp(table[j - 1].ptr, table[j].ptr, len1 * wchar.sizeof);
	...
	if (result == -1)
	{
	    ...
	    {
		if (table[i].length == ca.length)
		{   c = runtime.system.system.memcmp(table[i].ptr, ca.ptr, ca.length * wchar.sizeof);
		    assert(c != 0);
		}
	    }
	}
    ...
    */
	...
	while (low < high)
	{
	    ...
	    if (c == 0)
	    {
		c = runtime.system.system.memcmp(ca.ptr, pca.ptr, ca.length * wchar.sizeof);
		if (c == 0)
		{   //printf("found %d\n", mid);
		    return mid;
		}
...
int _d_switch_dstring(dchar[][] table, dchar[] ca)
	...
	for (j = 1; j < table.length; j++)
	{
		...
		c = runtime.system.system.memcmp(table[j - 1].ptr, table[j].ptr, len1 * dchar.sizeof);
		assert(c < 0);	// c==0 means a duplicate
	    }
	}
    }
	...
	if (result == -1)
	{
	    ...
	    {
		if (table[i].length == ca.length)
		{   c = runtime.system.system.memcmp(table[i].ptr, ca.ptr, ca.length * dchar.sizeof);
		    assert(c != 0);
		}
	    }
	}
    ...
    */
	...
	while (low < high)
	{
	    ...
	    if (c == 0)
	    {
		c = runtime.system.system.memcmp(ca.ptr, pca.ptr, ca.length * dchar.sizeof);
		if (c == 0)
		{   //printf("found %d\n", mid);
		    return mid;
		}
