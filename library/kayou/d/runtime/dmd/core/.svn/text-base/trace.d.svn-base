...
// XXX static FILE* fplog;

static string trace_deffilename = "trace.def";
// XXX static FILE* fpdef;
...
{   void *p;
    ...
    p = runtime.system.system.malloc(nbytes);
    if (!p)
...
static Symbol* trace_addsym(const(char)[] id)
{
    ...
    {
	cmp = runtime.core.util.string.compare(id, rover.Sident);
	if (cmp == 0)
	{
	    return rover;
	}
    ...
    runtime.system.system.memset(s,0,Symbol.sizeof);
    s.Sident = id.idup;
