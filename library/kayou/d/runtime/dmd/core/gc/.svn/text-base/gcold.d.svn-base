...
module runtime.core.gc.gcold;

//debug = PRINTF;

static import runtime.core.gc.gc;
...
ulong _d_newarrayi(size_t length, size_t size, ...)
{
	...
	runtime.core.util.va.va_start!(size_t)(q, size);		// q is pointer to ... initializer
	p = runtime.core.gc.gc._gc.malloc(length * size + 1);
	//debug(PRINTF) printf(" p = %p\n", p);
	if (size == 1)
	    runtime.system.system.memset(p, *cast(ubyte*)q, length);
	...
	else
	{
	    for (uint u = 0; u < length; u++)
	    {
		runtime.system.system.memcpy(p + u * size, q, size);
	    }
	}
...
ulong _d_newarrayii(size_t length, size_t size, size_t isize ...)
{
	...
	//void* q = cast(void*)(&size + 1);	// pointer to initializer
        runtime.core.util.va.va_list q;
	runtime.core.util.va.va_start!(size_t)(q, isize);		// q is pointer to ... initializer
	size *= length;
	p = runtime.core.gc.gc._gc.malloc(size * isize + 1);
	//debug(PRINTF) printf(" p = %p\n", p);
	if (isize == 1)
	...
	else
	{
	    for (uint u = 0; u < size; u++)
	    {
		runtime.system.system.memcpy(p + u * isize, q, isize);
	    }
	}
	runtime.core.util.va.va_end(q);
	result = cast(ulong)length + (cast(ulong)cast(uint)p << 32);
...
ulong _d_newm(size_t size, int ndims, ...)
{
    ...
    //debug(PRINTF) printf("_d_newm(size = %d, ndims = %d)\n", size, ndims);
    if (size == 0 || ndims == 0)
	result = 0;
    else
      {
	...
	void[] foo(size_t* pdim, int ndims)
	{
	    ...
	    if (ndims == 1)
	    {	p = runtime.core.gc.gc._gc.malloc(dim * size + 1)[0 .. dim];
		runtime.system.system.memset(p.ptr, 0, dim * size + 1);
	    }
	    else
	    {
		p = runtime.core.gc.gc._gc.malloc(dim * (void[]).sizeof + 1)[0 .. dim];
		for (int i = 0; i < dim; i++)
		{
...
ulong _d_newarraymi(size_t size, int ndims, ...)
{
	...
	void[] foo(size_t* pdim, int ndims)
	{
	    ...
	    {	p = runtime.core.gc.gc._gc.malloc(dim * size + 1)[0 .. dim];
		if (size == 1)
		    runtime.system.system.memset(p.ptr, *cast(ubyte*)pinit, dim);
		else
		{
		    for (size_t u = 0; u < dim; u++)
		    {
			runtime.system.system.memcpy(p.ptr + u * size, pinit, size);
		    }
		}
	    }
	    else
...
ulong _d_new(size_t length, size_t size)
{
	...
	p = runtime.core.gc.gc._gc.malloc(length * size + 1);
	//debug(PRINTF) printf(" p = %p\n", p);
	runtime.system.system.memset(p, 0, length * size);
	result = cast(ulong)length + (cast(ulong)cast(uint)p << 32);
    }
    return result;
}
...
extern (C)
byte[] _d_arraysetlength(size_t newlength, size_t sizeelem, runtime.core.gc.gc.Array *p)
in
{
    assert(sizeelem);
...
body
{
    ...
    */
	...
	if (p.data)
	{
		...
		size_t cap = runtime.core.gc.gc._gc.capacity(p.data);

		if (cap <= newsize)
		{
		    newdata = cast(byte *)runtime.core.gc.gc._gc.malloc(newsize + 1);
	...
	{
	    newdata = cast(byte *)runtime.core.gc.gc._gc.calloc(newsize + 1, 1);
	}
    }
    else
    {
 ...
 */
extern (C)
byte[] _d_arraysetlength2(size_t newlength, size_t sizeelem, runtime.core.gc.gc.Array *p, ...)
in
{
    assert(sizeelem);
    assert(!p.length || p.data);
...
body
{
    ...
    */
	...
	runtime.core.util.va.va_list q;
	runtime.core.util.va.va_start!(runtime.core.gc.gc.Array *)(q, p);	// q is pointer to initializer
	...
	if (newsize > size)
	{
		...
		{
		    runtime.system.system.memcpy(newdata + u, q, sizeelem);
		}
	    }
	}
    }
...
ulong _d_newbitarray(size_t length, bit value)
{
	...
	p = runtime.core.gc.gc._gc.malloc(size);
	//debug(PRINTF) printf(" p = %p\n", p);
	runtime.system.system.memset(p, fill, size);
	result = cast(ulong)length + (cast(ulong)cast(uint)p << 32);
    }
    return result;
}
...
{
extern (C)
bit[] _d_arraysetlengthb(size_t newlength, runtime.core.gc.gc.Array *p)
{
    byte* newdata;
    size_t newsize;
...
{
extern (C)
long _d_arrayappendb(runtime.core.gc.gc.Array *px, bit[] y)
{

    size_t cap = runtime.core.gc.gc._gc.capacity(px.data);
	...
	//newdata = runtime.core.gc.gc._gc.malloc(newlength * size);
	newdata = runtime.core.gc.gc._gc.malloc(runtime.core.gc.gc.newCapacity(newsize, 1) + 1);
	runtime.system.system.memcpy(newdata, px.data, (length + 7) / 8);
	px.data = cast(byte*)newdata;
    }
    px.length = newlength;
    if ((length & 7) == 0)
	// byte aligned, straightforward copy
	runtime.system.system.memcpy(px.data + length / 8, y, (y.length + 7) / 8);
    else
 ...
 */
extern (C)
byte[] _d_arraysetlength3(size_t newlength, size_t sizeelem, runtime.core.gc.gc.Array *p,
	size_t initsize, ...)
in
{
    assert(sizeelem);
...
body
{
    ...
    */
	...
	runtime.core.util.va.va_list q;
	runtime.core.util.va.va_start!(size_t)(q, initsize);	// q is pointer to initializer
...
extern (C)
long _d_arrayappend(runtime.core.gc.gc.Array *px, byte[] y, size_t size)
{
	...
	newdata = cast(byte *)runtime.core.gc.gc._gc.malloc(runtime.core.gc.gc.newCapacity(newlength, size) + 1);
	runtime.system.system.memcpy(newdata, px.data, length * size);
	px.data = newdata;
    }
    px.length = newlength;
    runtime.system.system.memcpy(px.data + length * size, y.ptr, y.length * size);
    return *cast(long*)px;
}
...
{
    size_t cap = runtime.core.gc.gc._gc.capacity(x.ptr);
	...
	cap = runtime.core.gc.gc.newCapacity(newlength, size);
	assert(cap >= newlength * size);
	newdata = cast(byte *)runtime.core.gc.gc._gc.malloc(cap + 1);
	runtime.system.system.memcpy(newdata, x.ptr, length * size);
    ...
    *cast(size_t *)&x = newlength;
    ...
    assert(runtime.core.gc.gc._gc.capacity(x.ptr) > x.length * size);
    return x;
}
...
out (result)
{
    ...
    size_t cap = runtime.core.gc.gc._gc.capacity(result.ptr);
...
body
{
    ...
    byte* p = cast(byte*)runtime.core.gc.gc._gc.malloc(len + 1);
...
{   byte[] a;
	...
	if (b.length)
	{
	    runtime.system.system.memcpy(&a[j], b.ptr, b.length * size);
	    j += b.length * size;
	}
    }
...
{   bit[] a;
    ...
    runtime.system.system.memcpy(a.ptr, x.ptr, x_bytes);
    if ((x.length & 7) == 0)
	runtime.system.system.memcpy(cast(void*)a.ptr + x_bytes, y.ptr, (y.length + 7) >> 3);
    else
...
bit[] _d_arraycopybit(bit[] from, bit[] to)
{
	    ...
	    cast(void *)from + nbytes <= cast(void *)to)
	{
	    nbytes = to.length / 8;
	    if (nbytes)
		runtime.system.system.memcpy(cast(void *)to, cast(void *)from, nbytes);
...
bit[] _d_arraysetbit2(bit[] ba, bit value)
{
    ...
    runtime.system.system.memset(ba.ptr, val, len >> 3);
...
void* _d_arrayliteral(size_t size, size_t length, ...)
{
	...
	*cast(size_t *)&result = length;	// jam length
	...
	if (stacksize == size)
	{
	    runtime.system.system.memcpy(result.ptr, q, length * size);
	}
	else
	{
	    for (size_t i = 0; i < length; i++)
	    {
		runtime.system.system.memcpy(result.ptr + i * size, q, size);
		q += stacksize;
	    }
	}
...
extern (C) long _adDup(runtime.core.gc.gc.Array2 a, int szelem)
    out (result)
    {
	assert(runtime.system.system.memcmp((*cast(runtime.core.gc.gc.Array2*)&result).ptr, a.ptr, a.length * szelem) == 0);
    }
    body
    {
	runtime.core.gc.gc.Array2 r;
...
version (none)
{
extern (C) long _adDupBit(runtime.core.gc.gc.Array a)
    out (result)
    {
      assert(runtime.system.system.memcmp((*cast(runtime.core.gc.gc.Array*)(&result)).ptr, a.ptr, (a.length + 7) / 8) == 0);
    }
    body
    {
	runtime.core.gc.gc.Array r;

	auto size = (a.length + 31) / 32;
	r.ptr = cast(void *) new uint[size];
	r.length = a.length;
	runtime.system.system.memcpy(r.ptr, a.ptr, size * uint.sizeof);
	return *cast(long*)(&r);
    }
