...
{   int result;
  ...
  //debug(apply) printf("_aApplyRcd1(), len = %d\n", aa.length);
	...
	{   char c = cast(char)d;
	    ...
	    while ((c & 0xC0) != 0xC0)
	    {	if (i == 0)
		    throw new runtime.core.util.utf.UtfException("Invalid UTF-8 sequence", 0);
		i--;
		d |= (c & 0x3F) << j;
		j += 6;
		m >>= 1;
...
{   int result;
  ...
  //debug(apply) printf("_aApplyRwd1(), len = %d\n", aa.length);
	...
	{   if (i == 0)
		throw new runtime.core.util.utf.UtfException("Invalid UTF-16 sequence", 0);
	    i--;
...
{   int result;
  ...
  //debug(apply) printf("_aApplyRwc1(), len = %d\n", aa.length);
	...
	{
	    char[4] buf;
	    auto b = runtime.core.util.utf.toUTF8(buf, d);
	    foreach (char c2; b)
	    {
