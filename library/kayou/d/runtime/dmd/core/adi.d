...
static import runtime.core.util.utf;
...
extern (C) long _adReverseChar(char[] a)
{
	...
	{   auto clo = *lo;
		...
		*hi = clo;
	    ...
	    uint stridelo = runtime.core.util.utf.UTF8stride[clo];
		...
		runtime.system.system.memcpy(tmp.ptr, lo, stridelo);
		runtime.system.system.memcpy(lo, hi, stridelo);
		runtime.system.system.memcpy(hi, tmp.ptr, stridelo);
		lo += stridelo;
		hi--;
		continue;
	    }
	     ...
	     */
	    runtime.system.system.memcpy(tmp.ptr, hi, stridehi);
	    runtime.system.system.memcpy(tmplo.ptr, lo, stridelo);
	    runtime.system.system.memmove(lo + stridehi, lo + stridelo , (hi - lo) - stridelo);
	    runtime.system.system.memcpy(lo, tmp.ptr, stridehi);
	    runtime.system.system.memcpy(hi + stridehi - stridelo, tmplo.ptr, stridelo);
...
unittest
{
    ...
    //printf("[kayou] unittest for adi.d\n");
...
extern (C) long _adReverseWchar(wchar[] a)
{
	...
	{   auto clo = *lo;
		...
		*cast(int*)hi = stmp;
	    ...
	    /* Shift the whole array. This is woefully inefficient
	     */
	    runtime.system.system.memcpy(tmp.ptr, hi, stridehi * wchar.sizeof);
	    runtime.system.system.memcpy(hi + stridehi - stridelo, lo, stridelo * wchar.sizeof);
	    runtime.system.system.memmove(lo + stridehi, lo + stridelo , (hi - (lo + stridelo)) * wchar.sizeof);
	    runtime.system.system.memcpy(lo, tmp.ptr, stridehi * wchar.sizeof);
...
extern (C) long _adReverse(Array a, size_t szelem)
	...
	if (a.length >= 2)
	{
	    ...
	    {
		//version (Win32)
		    tmp = cast(byte*) runtime.system.system.alloca(szelem);
		//else
		    //tmp = new byte[szelem];
	    }
	    ...
	    {
		runtime.system.system.memcpy(tmp, lo,  szelem);
		runtime.system.system.memcpy(lo,  hi,  szelem);
		runtime.system.system.memcpy(hi,  tmp, szelem);
	    }
...
extern (C) long _adSortChar(char[] a)
{
    if (a.length > 1)
    {
	dstring da = runtime.core.util.utf.toUTF32(a);
	da.sort;
	size_t i = 0;
	foreach (dchar d; da)
	{   char[4] buf;
	    auto t = runtime.core.util.utf.toUTF8(buf, d);
	    a[i .. i + t.length] = t[];
	    i += t.length;
	}
	delete da;
...
extern (C) long _adSortWchar(wchar[] a)
{
	...
	foreach (dchar d; da)
	{   wchar[2] buf;
	    auto t = runtime.core.util.utf.toUTF16(buf, d);
	    a[i .. i + t.length] = t[];
	    i += t.length;
	}
	delete da;
...
+/
	...
	return (runtime.system.system.memcmp(p1, p2, a1.length) == 0);
...
extern (C) int _adCmp(Array a1, Array a2, TypeInfo ti)
{
    ...
    if (sz == 1)
    {	// We should really have a ti.isPOD() check for this
	auto c = runtime.system.system.memcmp(p1, p2, len);
	if (c)
	    return c;
    }
    else
...
else
{
    ...
    if (a2.length < len)
	len = a2.length;
    c = runtime.system.system.memcmp(cast(char *)a1.ptr, cast(char *)a2.ptr, len);
    if (!c)
